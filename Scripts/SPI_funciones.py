...
def media_anomalia(shpcuenca,ChirpsUTM_clipped_folder,anho_inicio,anho_fin):
    ...
    for i in list3:
        if int(i[-8:-4]) >= anho_inicio & int(i[-8:-4]) <= anho_fin:
            ...
            affine2 = input_raster.GetGeoTransform()
            input_raster = None
            std = 1/len(precip_hist)+(precip_hist[i[-8:-4]] - mean)^2 + std
    std2 = std^0.5

    new_affine2 = Affine(affine2[1],affine2[2],affine2[0],affine2[4],affine2[5],affine2[3])
    stats = zonal_stats(shp, mean, affine=new_affine2, stats=["mean"], all_touched=True)  # se asignan los valores maximos en la intersección con el shapefile
    stats2b = zonal_stats(shp, std2, affine=new_affine2, stats=["mean"], all_touched=True)  # se asignan los valores maximos en la intersección con el shapefile
